...
console.log(5 - 1); // 5에서 1을 뺴서 1이라는 새로운 값을 생성.
console.log(10 > 4); // 10과 4를 비교해서 true 라는 새로운 값 생성.
...
/*
    ...
    전위 연산자 : 피연산자의 앞에 사용. => 우선순위가 가장 먼저이다.
...
//비교연산자 : 좌항과 우항을 비교해 true 혹은 false 를 반환
...
/* 
    논리 연산자 : 좌항과 우항의 논리값을 조합해서 새로운 논리값을 생성.
        => 여러 개의 조건을 한번에 확인할 때 사용한다. 
    ||(or) : 둘 중 하나라도 true => true, 둘 모두 false 일 때만 false
    && (and) : 둘 모두 true 일 때만 true, 둘 중 하나라도 false 면 false.
*/
...
//      => +를 사용한다. => 피연산자 문자열이며 연결 연산자, 숫자면 산술 연산자.
...
console.log(1 + "1"); // 숫자 1은 문자열로 암묵적 형변환
// typeof 연산자 : 피연사의 데이터 타입을 반환한다.
console.log(typeof "안녕하세요");

// 문자열을 숫자로 바꾼다 -> 숫자로 변호나 가능한 문자열만 바꾼다.
